    ...
    if not iframe:
        ...
        if inputFileName.endswith('.md'):
            ...
            for line in text.splitlines():
                ...
                if line.startswith('<!--StyleStart-->'):
                    htmlStyle = ''
                    inStyle = True
                elif line.startswith('<!--StyleEnd-->'):
                    inStyle = False
                    break
                elif inStyle:
                    # Split the line into words
...
def main():
    ...
    # Add top folder
    parser.add_argument('-tp', '--top', dest='topfoloder', default="..", help='Top folder. Default is ..')
    # Add doc folder
    ...
    for filePair in filePairs:
        ...
        convertToHtml(filePair[0], filePair[1], opts.template, opts.iframe, opts.topfoloder, opts.docfolder)
